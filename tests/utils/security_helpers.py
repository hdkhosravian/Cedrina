...
class SecurityTestHelpers:
    ...
    def create_test_user(user_id: int, username: str = None, email: str = None) -> User:
        ...
        return User(id=user_id, username=username, email=email, role=Role.USER, is_active=True)
    ...
    def create_expired_token(user: User, days_ago: int = 1) -> str:
        ...
        return SecurityTestHelpers.create_jwt_token(
            user=user, jti=f"expired-{user.id}", exp_delta=timedelta(days=-days_ago)
        )
    ...
    def create_wrong_issuer_token(user: User, malicious_issuer: str = "https://evil.com") -> str:
        ...
        """
        return SecurityTestHelpers.create_jwt_token(
            user=user, jti=f"wrong-issuer-{user.id}", issuer=malicious_issuer
        )
    ...
    def create_wrong_audience_token(user: User, wrong_audience: str = "wrong-audience") -> str:
        ...
        """
        return SecurityTestHelpers.create_jwt_token(
            user=user, jti=f"wrong-audience-{user.id}", audience=wrong_audience
        )

    @staticmethod
    async def validate_token_ownership_pattern(
        token: str, current_user: User, language: str = "en"
    ) -> Dict[str, Any]:
    ...
    def assert_cross_user_attack_prevented(
        primary_user: User, other_user: User, token_validation_func, create_token_func=None
    ):
...
class SecurityAssertions:
    ...
    @staticmethod
    def assert_secure_logging(
        log_capture, expected_event: str, expected_fields: Dict[str, Any] = None
    ):
        ...
        matching_entries = [
            entry for entry in log_capture.entries if expected_event in entry.get("event", "")
        ]
        ...
        if expected_fields:
            for field_name, expected_value in expected_fields.items():
                assert (
                    field_name in matching_entries[0]
                ), f"Expected field '{field_name}' not found in log"
                assert (
                    matching_entries[0][field_name] == expected_value
    ...
    def assert_consistent_error_response(response_one, response_two):
        ...
        """
        assert (
            response_one.status_code == response_two.status_code
        ...
        assert (
            "Invalid" in error_one or "Authentication" in error_one
        ), "Error message should be generic"
        assert (
            "Invalid" in error_two or "Authentication" in error_two
        ), "Error message should be generic"
